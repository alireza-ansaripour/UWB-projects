 ...
 *
 * NOTE: Callbacks can be undefined (set to NULL). In this case, 
...
void ali_setcallbacks(dwt_cb_t cbSFDDone){
  pdw3000local->cbSFDOk = cbSFDDone;
}
...
void ali_isr(void)
{
      ...
      uint32_t rx_t;
      if(status & SYS_ENABLE_LO_RXPRD_ENABLE_BIT_MASK){
              gpio_set(LED_RX);
              gpio_set(PORT_DE);
              dwt_write32bitreg(SYS_STATUS_ID, SYS_STATUS_RXPRD_BIT_MASK);
              if(pdw3000local->cbPREOk)
              	pdw3000local->cbPREOk(&pdw3000local->cbData);
      }
      if(status & SYS_ENABLE_LO_RXSFDD_ENABLE_BIT_MASK){
              gpio_reset(PORT_DE);
              dwt_write32bitreg(SYS_STATUS_ID, SYS_STATUS_RXSFDD_BIT_MASK);
              //rx_t = dwt_readrxtimestamphi32();
              //printf("%d\n", rx_t);              
              //if(pdw3000local->cbSFDOk)
              //	pdw3000local->cbSFDOk(&pdw3000local->cbData);
      }
      if (status & SYS_ENABLE_LO_RXFCG_ENABLE_BIT_MASK){
              gpio_reset(LED_RX);
              dwt_write32bitreg(SYS_STATUS_ID, SYS_STATUS_ALL_RX_GOOD); // Clear all status bits relating to good reception
              uint16_t finfo16;
      ...
      if(status & SYS_ENABLE_LO_TXFRB_ENABLE_BIT_MASK){
              gpio_set(PORT_DE);
              gpio_set(LED_TX);
              dwt_write8bitoffsetreg(SYS_STATUS_ID, 0, (uint8_t)SYS_STATUS_ALL_TX);

      }
      if(status & SYS_ENABLE_LO_TXPRS_ENABLE_BIT_MASK){
              gpio_reset(PORT_DE);
      }
      if(status & SYS_ENABLE_LO_TXFRS_ENABLE_BIT_MASK){
              gpio_reset(LED_TX);
              dwt_write8bitoffsetreg(SYS_STATUS_ID, 0, (uint8_t)SYS_STATUS_ALL_TX);
